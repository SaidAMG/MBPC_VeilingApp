...
namespace MBPC_VeilingApp.Classes
{
    internal class Sort
    {
        public class LotSorter : IComparer<List<object>>
            {
            ...
            {
                List<Lot> sortedLotsByAuctionId = lots.OrderBy(lot => lot.GetAuctionId().GetId()).ToList();
                List<Lot> sortedLotsByName = sortedLotsByAuctionId.OrderBy(lot =>
                {
...
//    // Vervolgens sorteren op naam (name) vanuit de class Booklet
//    List<Lot> sortedLotsByName = sortedLotsByAuctionId.OrderBy(lot => lot.GetBookletId().GetName()).ToList();
